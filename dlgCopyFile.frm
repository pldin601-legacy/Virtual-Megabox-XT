...
Begin VB.Form dlgCopyFile 
   ...
   Begin VB.CheckBox chkDelete 
      BackColor       =   &H00404040&
      Caption         =   "Delete source after copy"
      Enabled         =   0   'False
      ForeColor       =   &H0000FFFF&
      Height          =   555
      Left            =   3360
   ...
   Begin VB.CheckBox chkRePath 
      BackColor       =   &H00404040&
      Caption         =   "Readdress files after copying"
      CausesValidation=   0   'False
      ForeColor       =   &H0000FFFF&
      Height          =   375
      Left            =   3360
...
Function CopyFile(ByVal SourcePath As String, ByVal DestinationPath As String, ByVal SFileName As String, ByVal DFileName As String) As Boolean
    ...
    If LOF(1) > GotFreeDiskSpace(Left(lstCopyPath.Path, 3)) Then MsgBox "Disk has no free space!", vbCritical: GoTo ErrorCopy
...
CPath = InputBox("Enter the folder name:", "Create Directory", "Default")
If CPath = "" Then Exit Sub
MkDir LowPath(lstCopyPath.Path) + CPath
If Err Then MsgBox "Error during creating"
lstCopyPath.Refresh
...
For A = 0 To lstFiles.ListCount - 1
 lblCopyState.Caption = "Copying " & (A + 1) & " of " _
 & lstFiles.ListCount
 DoEvents
 ...
 If lstFiles.Selected(A) = True Then
   ...
   If X = True Then
     ...
     End If
   Else
     MsgBox "Errors found!", vbCritical: Exit Sub
   End If
   
   If Err Then
     B = MsgBox(Language("COPY_ERROR:"), vbCritical + vbYesNo)
     Err = 0
