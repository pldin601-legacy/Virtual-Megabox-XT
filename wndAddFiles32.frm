...
OPath = InputBox("Enter the folder name and path here:", "Go to folder", lstDir.Path)
lstDir.Path = OPath
...
If Me.Tag <> "Open" Then
 lblCaption.Caption = "Add files..."
Else
 lblCaption.Caption = "Open files..."
End If
...
On Error Resume Next
WinSeek.txtSearchSpec.Text = Me.txtFileName.Text
WinSeek.dirList.Path = Me.lstDir.Path
WinSeek.Show 1, Me
Me.Hide
End Sub
...
If Err Then
 ...
 MsgBox "Drive ''" + lstDrive.Drive + "''not ready!", vbExclamation
End If
...
If Me.Tag = "Open" Then wndMain.lstMain.Clear: wndMain.lstSec.Clear
...
For X = 0 To lstFile.ListCount - 1
 If lstFile.Selected(X) = True Then
  ...
  Else
    ...
    wndMain.lstSec.AddItem LowPath(lstFile.Path) + lstFile.List(X)
  End If
 End If
Next

wndMain.UpdateTags -1
